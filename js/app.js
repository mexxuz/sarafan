(function () {
  ...
  const FOCUS_HINT = {
    ...
    pediatr: ['неонатолог', 'детский ЛОР', 'детский невролог', 'детский аллерголог', 'детский эндокринолог'],
    therapist: ['эндокринолог', 'гастроэнтеролог', 'невролог', 'ЛОР', 'дерматолог', 'гинеколог', 'уролог'],
    cardio: ['аритмолог', 'детский кардиолог', 'УЗИ сердца'],
    psych: ['семейный', 'детский', 'тревога и паника', 'зависимости', 'пары'],
    lawyer: ['семейное право', 'трудовые споры', 'недвижимость', 'бизнес и договоры', 'уголовное'],
    tutor: ['математика', 'физика', 'русский язык', 'подготовка к экзаменам'],
    english: ['IELTS', 'для детей', 'разговорный', 'деловой'],
    repair: ['плитка', 'под ключ', 'покраска', 'гипсокартон'],
    auto: ['ходовая', 'двигатель', 'автоэлектрика', 'кузов'],
    beauty: ['маникюр', 'брови', 'стрижки', 'ресницы'],
    photo: ['свадьбы', 'семейные', 'предметная', 'репортаж'],
  };
  // Быстрые метки в рекомендации — то, что спрашивают о таких людях. По ним же ищут
  const MED = ['dentist', 'pediatr', 'therapist', 'cardio', 'psych'];
  const TAGS_MED = ['не назначает лишнего', 'объясняет понятно', 'хорош с детьми', 'можно написать после приёма', 'приезжает на дом', 'без очередей'];
  const TAGS_ALL = ['делает в срок', 'честные цены', 'отвечает быстро', 'аккуратно, убирает за собой', 'приезжает в тот же день', 'объясняет понятно'];
  const tagsFor = (c) => (MED.includes(c) ? TAGS_MED : TAGS_ALL);
  ...
  // Ссылка — кнопкой соцсети: значок, название и ник, без хвостов вроде «?igsh=…» (правка 25.09).
  // Значки — из открытой библиотеки брендов Simple Icons (simple-icons@13), не нарисованы по памяти
  const SOC_ICON = {
  ...
  const SOC = [
    [/(^|\.)instagram\.com$/, 'instagram', 'Instagram', '#E4405F'], [/^(t|telegram)\.me$/, 'telegram', 'Telegram', '#26A5E4'],
    [/(^|\.)facebook\.com$|^fb\.com$/, 'facebook', 'Facebook', '#0866FF'], [/(^|\.)tiktok\.com$/, 'tiktok', 'TikTok', '#000000'],
  ...
  function linkBtn(raw) {
    ...
    if (!href0) return '';
    let url;
    try { url = new URL(/^https?:\/\//i.test(href0) ? href0 : 'https://' + href0); } catch (e) { return ''; }
    const host = url.hostname.replace(/^www\./, '').toLowerCase();
    const soc = SOC.find(([re]) => re.test(host));
    const path = url.pathname.replace(/\/+$/, '');
    let href = url.href, label, sub;
    if (soc) {
      href = url.origin + path;                                   // ссылка без меток «?igsh=…», «?stkn=…»
      const handle = path.split('/').filter(Boolean)[0] || '';
      // название соцсети не пишем — его и так видно по значку; только ник
      label = soc[1] === 'youtube' ? (handle.startsWith('@') ? handle : soc[2]) : handle ? '@' + handle.replace(/^@/, '') : soc[2];
      sub = '';
    } else {
      label = host;
      sub = path && path !== '/' ? decodeURIComponent(path).slice(0, 40) : 'сайт';
    }
    const icon = soc && SOC_ICON[soc[1]]
      ? `<svg class="soc-ic" viewBox="0 0 24 24" style="color:${soc[3]}" aria-hidden="true"><path fill="currentColor" d="${SOC_ICON[soc[1]]}"/></svg>`
      : ic('link');
...
`;
  ...
  function cloudData(limitRing, withPlaces, maxFar, onlyPlaces, sky) {
    ...
    if (!onlyPlaces && (sky || people.length > 60)) {
      ...
      // места, которые вы сами завели или советуете (и ваши ближние знакомые), — тоже значками, а не безымянными звёздами
      // (правка 25.09: бар Славы в его сети был еле видной звёздочкой)
      nodes.filter((n) => isPlace(n.id)).forEach((n) => {
        const nd = nodeById(n.id.slice(1));
        if (!nd) return;
        const voices = [nd.by, ...nodeRecs(nd).map((r) => r.from)];
        if (voices.some((v) => v === S.me || (v && ring(v) <= 1))) core.add(n.id);
      });
      nodes.forEach((n) => {
  ...
  // Всё, что умеет бот в чатах, — по местам: в любой переписке, в группах, в личке (правка 26.09: дополнено)
  function sheetChatHelp() {
    const b = esc(S.bot || 'sarafanibot');
    const row = (icon, title, text) => `<div class="help-row">${ic(icon)}<div><b>${title}</b><span>${text}</span></div></div>`;
    openSheet({
      F: {},
      render: () => `${sheetHead(null, 'Сарафан в чатах', 'Советовать не выходя из переписки')}
        <div class="eyebrow" style="margin-top:4px">в любой переписке — наберите <code>@${b}</code></div>
        <div class="help-list">
          ${row('seal', 'Посоветовать человека', `Допишите сферу: <code>@${b} педиатр</code> — и отправьте карточку. Видно, кто рекомендует; под ней «Сохранить себе»`)}
          ${row('user', 'Моя карточка', 'Ваша визитка с работами — отправить клиенту или в чат')}
          ${row('send', 'Позвать в Сарафан', 'Приглашение от вашего имени: кто войдёт, станет вашим знакомым')}
          ${row('spark', 'Как работает Сарафан', 'Короткая галерея — показать другу, что это')}
          ${row('plus', 'Сохранить сообщение', `Ответьте на сообщение с контактом и наберите <code>@${b}</code> — оно уйдёт в ваш Сарафан`)}
        </div>
        <div class="eyebrow" style="margin-top:16px">в группах — добавьте бота участником</div>
        <div class="help-list">
          ${row('plus', 'Кнопка под советом', 'Кто-то ответил телефоном или контактом — бот ставит «Сохранить себе», через сутки убирает')}
          ${row('chat', 'Ответы — в личку', 'Спросили в группе «посоветуйте…» — через час бот пришлёт все советы из ответов')}
        </div>
        <div class="eyebrow" style="margin-top:16px">в личной переписке</div>
        <div class="help-list">
  ...
  const jobDays = (x) => (x.days ? ' · ' + x.days : '');
  function sheetWorkDays(linkId) {
    const x = (S.nodePeople || []).find((y) => y.id === linkId);
    if (!x) return;
    const n = nodeById(x.node);
    const parts = (x.days || '').split('·');
    const f = { days: WEEK.filter((d) => (parts[0] || '').includes(d)), hours: (parts[1] || '').trim() };
    openSheet({
      F: f,
      render: () => `${sheetHead(null, 'Дни приёма', esc(n ? n.name : ''))}
        <div class="field" style="margin-top:0"><span>Когда вы здесь</span><div class="chips">${WEEK.map((d) => `<button class="chip ${f.days.includes(d) ? 'on' : ''}" data-act="toggle" data-k="days" data-v="${d}">${d}</button>`).join('')}</div></div>
        <label class="field"><span>Часы, если хотите</span><input class="input" data-bind="hours" maxlength="30" placeholder="9:00–14:00" value="${esc(f.hours)}"></label>
        <p class="why">${ic('spark')}Кто вас советует, увидит: «Дент-Люкс · пн, ср» — и придёт в нужный день</p>
        <div class="s-foot"><button class="btn primary block" data-act="saveWorkDays" data-id="${linkId}">Сохранить</button></div>`,
      save: () => {
        const days = [WEEK.filter((d) => f.days.includes(d)).join(', '), f.hours.trim()].filter(Boolean).join(' · ');
        closeAllSheets();
        mutate(() => { x.days = days; }, '/nodes/people/days', { id: linkId, days }, days ? 'Дни приёма сохранены' : 'Дни приёма убраны');
      },
    });
  }
  ...
  function voicesView(id) {
    const anon = (U(id) || {}).anonRecs || [];
    const recs = G.recsTo(id);
    const count = {};
  ...
  const TOUR = [
    {
      ...
      title: 'Нужен педиатр? Спросите своих',
      gain: 'Ваш знакомый Азиз водит дочку к педиатру Нигоре — вы видите её, что о ней говорят и через кого она. Пишете ей напрямую, без рекламы и случайных отзывов',
  ...
  function Onboarding() {
    ...
    return `<div class="onb">
      ...
      ['seek', 'search', 'Ищу проверенного человека', 'врача, мастера, юриста — через своих']]
  ...
  function sheetRecommend(id, catId) {
    ...
    const f = { cat: catId || prefer[0] || '', rel: '', text: '', interest: '', priv: false, allCats: !prefer.length, tags: [], anon: false };
    const existing = () => G.recsFrom(S.me).find((r) => r.to === id && r.cat === f.cat) || S.recs.find((r) => r.from === S.me && r.to === id && r.cat === f.cat);
    const fillFromExisting = () => { const e = existing(); if (e) { f.text = e.text; f.rel = e.rel; f.interest = e.interest || ''; f.priv = !!e.private; f.tags = [...(e.tags || [])]; f.anon = !!e.anon; } };
    fillFromExisting();
    openSheet({
      F: f,
      onSet: (k) => { if (k === 'cat') { f.text = ''; f.rel = ''; f.tags = []; fillFromExisting(); } },
    ...
    openSheet({
      ...
      submit: () => {
        const body = { name: f.name.trim(), about: f.about.trim(), role: f.role, availability: f.avail,
          ...
          focus: Object.fromEntries((f.role === 'client' ? [] : f.cats).map((c) => [c, (f['focus_' + c] || '').trim()])) };
        closeSheet();
        mutate(() => { S.availability = f.avail; S.cardPhone = body.cardPhone; Object.assign(me, { name: body.name, about: body.about, cats: body.cats, role: body.role, busy: f.avail === 'busy', hidden: f.avail === 'hidden', focus: body.focus,
  ...
  const ACT = {
    ...
    focusPick: (d) => { const k = 'focus_' + d.c; const cur = (SH.F[k] || '').split(',').map((x) => x.trim()).filter(Boolean);
      if (!cur.includes(d.v)) cur.push(d.v); SH.F[k] = cur.join(', '); drawSheet(); },
